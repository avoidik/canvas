...
func initDatabase() {
	...
	sysfs := os.DirFS("../storage/migrations")
	if err := migrate.Up(context.Background(), db.DB.DB, sysfs); err != nil {
		panic(err)
	}
	if err := migrate.Down(context.Background(), db.DB.DB, sysfs); err != nil {
		panic(err)
	}
	if err := migrate.Up(context.Background(), db.DB.DB, sysfs); err != nil {
		panic(err)
